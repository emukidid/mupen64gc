...
static long long int comp_reg[34];
...
void display_error(char *txt)
{
   ...
   float *comp_float = (float*)comp_reg;
   double *comp_double = (double*)comp_reg;
   printf("err: %s\n", txt);
   ...
   if(!strcmp(txt, "cop1")){
   	for(i=0; i<32; i++){
   		if(reg_cop0[12] & 0x04000000){
   			printf("d%d: %20f %20f\n", i, *(reg_cop1_double[i]), comp_double[i]);
   			printf("f%d: %20f %20f\n", i, (double)*(reg_cop1_simple[i]), (double)comp_float[2*i]);
   		} else {
   			if(!(i&1))
	   			printf("d%d: %20f %20f\n", i, *(reg_cop1_double[i]), comp_double[i/2]);
   			printf("f%d: %20f %20f\n", i, (double)*(reg_cop1_simple[i]), (double)comp_float[i^1]);
   		}
   	}
   }
...
static int check_fp(void){
	float *comp_float = (float*)comp_reg;
	double *comp_double = (double*)comp_reg;
	int i;
	for(i=0; i<32; i++){
		if(reg_cop0[12] & 0x04000000){
			if( fabs(  *(reg_cop1_double[i]) - comp_double[i] ) > 0.1  &&
			    fabsf( *(reg_cop1_simple[i]) - comp_float[i]  ) > 0.1f ) return -1;
		} else {
			if( fabs( *(reg_cop1_double[i]) - comp_double[i/2] ) > 0.1  &&
			    (fabsf( *(reg_cop1_simple[i]) - comp_float[i^1] ) > 0.1f ||
			     fabsf( *(reg_cop1_simple[i+1]) - comp_float[i+1] ) > 0.1f) ) return -2;
			++i;
		}
   	}
   	return 0;
...
{   
   ...
   if (dynacore || !interpcore)
	...
	fread (comp_reg, 34, sizeof(long long int), f);
	if (memcmp(reg, comp_reg, 34*sizeof(long long int)))
	  display_error("gpr");
	...
	fwrite(&interp_addr, 4, sizeof(long), f);
	fwrite(reg, 34, sizeof(long long int), f);
