...
static void release(int minNeeded){
	...
	for(n = tail; n != NULL && minNeeded > 0;){
		printf("Releasing block %05x from RecompCache\n", n->blockNum);
		nodeRemove(n);
...
void* RecompCache_Alloc(unsigned int size, unsigned int blockNum){
	printf("RecompCache_Alloc(%d, %05x)\n", size, blockNum);
...
void* RecompCache_Realloc(void* memory, unsigned int size){
	printf("RecompCache_Realloc(%08x, %d)\n", memory, size);
	printf("(%dkB used of %dkB or %f\%)\n", cacheSize/1024, RECOMP_CACHE_SIZE/1024, ((float)cacheSize)/RECOMP_CACHE_SIZE*100);
...
void RecompCache_Free(unsigned int blockNum){
	printf("Freeing block %05x from RecompCache\n", blockNum);
