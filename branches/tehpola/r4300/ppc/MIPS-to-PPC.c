...
static int LB(MIPS_instr mips){
	...
	mapRegisterNew( MIPS_GET_RT(mips) ); // rt = r3
...
static int SB(MIPS_instr mips){
	...
	int rd   = mapRegister( MIPS_GET_RT(mips) ); // r3 = value
	...
	GEN_STB(ppc, rd, MIPS_GET_IMMED(mips), base);
	set_next_dst(ppc);
	// Skip over else
	GEN_B(ppc, 9, 0, 0);
	set_next_dst(ppc);
	...
	set_next_dst(ppc);
	// addr = base + immed
	GEN_ADDI(ppc, base, base, MIPS_GET_IMMED(mips));
	set_next_dst(ppc);
	// type = MEM_SB
...
static int SH(MIPS_instr mips){
	...
	set_next_dst(ppc);
	// Perform the actual save
	GEN_STH(ppc, rd, MIPS_GET_IMMED(mips), base);
	set_next_dst(ppc);
	// Skip over else
	GEN_B(ppc, 9, 0, 0);
	set_next_dst(ppc);
...
static int SW(MIPS_instr mips){
	...
	set_next_dst(ppc);
	// Perform the actual save
	GEN_STW(ppc, rd, MIPS_GET_IMMED(mips), base);
	set_next_dst(ppc);
	// Skip over else
	GEN_B(ppc, 9, 0, 0);
	set_next_dst(ppc);
